...
def incr(a, step=1): # a를 step만큼 증가시키는 함수
    # step의 기본값은 1, 호출시 step을 명시하지 않으면 기본값을 사용
    return a + step

print("incr:", incr(10,2))
print("incr(기본값 사용):", incr(10))    # 두번째 인수는 명시하지 않음 -> 기본값 사용
...
print("AREA:", area(height=20, width=10))   # 키워드 인수 사용시, 함수에 선언된 인수의 순서대로 써주지 않아도 괜찮음.
...
print(get_total(2, 4, 6.5, 8, 10))
print(get_total(2, 4, "6", 8, 10)) # 6은 합산되지 않음
...
def func_args(a, b, *c, **kwd):
    print(a, b)             # 고정인수
    print(c, type(c))       # 가변인수(튜플 형태)
    print(kwd, type(kwd))   # 사전 키워드 인수(딕션어리 형태)

func_args(10, 30, 30, 40, 50, option1="value1", option2="value2")



# 함수도 객체이므로 다른 함수의 인수로 넘겨줄 수 있음
def add(a, b):
    return a + b
...
print("calc + add:", calc(10, 20, add))
print("calc + subtract:", calc(10, 20, subtract))

# 예제
# 정제되지 않은 문자열이 있음..
words = "python pRogRammIng, jaVa pRoGRAMMing, LINUX, WinDoWS".split(",")
print("DIRTY Strings:", words, type(words))
# words 내부의 문자열들에 문자열 함수를 전달해주어서 정제하는 함수 작성
def clean_strings(strings, *funcs):
    result = []     # 빈 리스트
    for string in strings:
        # 함수의 목록을 루프
        for func in funcs:  # funcs는 가변인수라서 튜플로 넘어올 것임
            if callable(func): # 가변 인수로 넘어온 func가 함수면, 호출
                string = func(string)
