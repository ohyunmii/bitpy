# 파일 모드
# 파일 타입에 대한 구분: text (t-default), binary(b)
...
def read_multiline():
    ...
    print("data:", data)
    # 한번에 데이터를 불러올 경우,
    # 파일 사이즈가 클 경우 메모리 문제가 발생할 가능성이 있음
    lines = data.split("\n")    # split을 하면 list가 나올 것임
...
def read_multiline02():
    ...
    while True: # 무한 루프
        ...
        if len(line) == 0:
            ...
            lines.append(line.strip())  # strip은 \n 없애줌.
...
def with_as():
    # 파일, DBMS, 네트워크 등 시스템 자원은 open 하고 사용을 했으면
    # 반드시 닫아줘야 한다
...
def pickle_dump():
    # pickle은 바이트 스트림 형태로 직렬화 하므로 파일 모드는 반드시 binary 모드여야 함
    with open("./sample/players.bin", "wb") as f:
        ...
        #객체 직렬화
        pickle.dump(lst, f)
        # 원래 단일 객체 직렬화 용이지만, dump 여러번 하면 복수 개의 객체를 직렬화 할 수 있음
        pickle.dump(dct, f)
        pickle.dump(dct2, f, pickle.HIGHEST_PROTOCOL)
        # dump 시 프로토콜 버전을 명시할 수 있다

def pickle_load():
    # 역직렬화
    with open("./sample/players.bin", "rb") as f: # "rb" : read binary
        # dump 시 프로토콜 버전을 명시했더라도, 로드 시에는 명시하지 않아도 된다. 파일에 저장되어 있는 정보이기 때문.
        """
        print(pickle.load(f))
        print(pickle.load(f))
        print(pickle.load(f))
        ...
        # 더 많은 객체를 역직렬화 하고자 하면 -> EOFError 발생
        print(pickle_load(f))
         """
        # Pickle 객체를 저장할 리스트
        data_list = []
        ...
        while True:
            try:
                ...
                break # 더이상 역직렬화할 객체가 없음
            data_list.append(data)

        for obj in data_list:
            print("pickle object:", obj, type(obj))
